...
class MADDPG(RLC):
    def __init__(self, env: CityLearnEnv, actor_units: list = [256, 128], critic_units: list = [256, 128],
        ...
        self.lagrangian_logfile = "lagrangian_monitoring_New_Cost_inequality.csv"
    ...
    def get_constraint_cost(self, observations, actions, next_observations):
        ...
        # === SPSA-based constraint cost estimate for actor loss ===
    def estimate_constraint_spsa(self, agent_num, actions, epsilon=0.01):
        """
        Estimate constraint cost for actor loss using SPSA.
        Replaces the need for constraint_critic(obs, actions).
        """
        action_tensor = actions[agent_num].clone().detach()
        delta = torch.randn_like(action_tensor)
        plus_action = action_tensor + epsilon * delta
        minus_action = action_tensor - epsilon * delta
    
        g_plus = self.compute_constraint_cost(agent_num, plus_action.unsqueeze(0)).item()
        g_minus = self.compute_constraint_cost(agent_num, minus_action.unsqueeze(0)).item()
    
        return (g_plus + g_minus) / 2.0
